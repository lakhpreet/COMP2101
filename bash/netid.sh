...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    ;;
    * )
      if [ "$interfaceInputName" == "" ]; then
        interfaceInputName="$1"
        printIndividualInterface="yes"
      fi
    ;;
  esac
  shift
...
if [ $printIndividualInterface = "yes" ]; then
  [ "$verbose" = "yes" ] && echo "Retrieving all interfaces present in the system"
  allInterfaces=$(ip a | awk '/: e/{gsub(/:/,"");print $2}')
  allInterfaces=$(echo "$allInterfaces" | awk '{printf "%s ",$0} END {print ""}')
  totalInterfaceArray=($allInterfaces)
  totalNumberOfInterfaces=${#totalInterfaceArray[@]}
...
if [ "$interfaceInputName" != "" ]; then
      if [[ " ${totalInterfaceArray[@]} " =~ " $interfaceInputName " ]]; then
        totalInterfaceArray=($interfaceInputName)
        totalNumberOfInterfaces=${#totalInterfaceArray[@]}
        [ "$verbose" = "yes" ] && echo "Reporting just the interface selected by the user: $interfaceInputName"
      else
        echo "the interface \" $interfaceInputName \" that you are trying to reach do not exist in your system"
        exit
      fi
    else
      exit
    ...
    for (( count=0; count < $totalNumberOfInterfaces ; count++ )); do
        interface=${totalInterfaceArray[count]}
...
# Task 2
# printing each interface infromation
allInterfaces=$(ip a | awk '/: e/{gsub(/:/,"");print $2}')
allInterfaces=$(echo "$allInterfaces" | awk '{printf "%s ",$0} END {print ""}')
totalInterfaceArray=($allInterfaces)
totalNumberOfInterfaces=${#totalInterfaceArray[@]}
...
echo " 
Task 2:
Looping through each of ($totalNumberOfInterfaces) interface(s) and printing it! 
"
for (( count=0; count < $totalNumberOfInterfaces ; count++ )); do
    interface=${totalInterfaceArray[count]}
