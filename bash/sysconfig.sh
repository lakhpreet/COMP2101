...
cat <<EOF
  Syntax: sysconfig.sh [OPTION]...
  This script displays all system information but you can get specific information through options

  OPTIONS Available for this script:
    -h, --help         Shows help and then exit
    --host             Show the Hostname
    --domain           Show the Domain
    --ipconfig         Show IP Addresses and Interfaces
    --os               Show the OS name
    --cpu              Show CPU specifications
    --memory           Show the memory
    --disk             Show the disk name and space
    --printer          Show the Printer information
    Any other parameters will error-exit the script
EOF
...
function cleanup {
   echo
   sudo rm /tmp/*.$$
   echo "Deleted every temporary file created during script"
   echo
   exit
}
